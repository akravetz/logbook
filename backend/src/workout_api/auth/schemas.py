...
class DevLoginRequest(BaseModel):
    """Development mode login request."""

    email: EmailStr = Field(description="User's email address for development login")
    name: str | None = Field(
        default=None, description="Optional display name for the user"
    )


class DevLoginResponse(BaseModel):
    """Development mode login response."""

    user: UserProfileResponse = Field(description="User profile information")
    tokens: TokenResponse = Field(description="JWT token pair")
    dev_mode: bool = Field(
        default=True, description="Indicates this is a development login"
    )
